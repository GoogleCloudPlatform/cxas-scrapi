...
COMMANDS_TO_BENCHMARK = [
    ...
    (
        "cxas trace search --help",
        ["uv", "run", "cxas", "trace", "search", "--help"],
    ),
    ("cxas evals --help", ["uv", "run", "cxas", "evals", "--help"]),
    (
        "cxas evals report --help",
        ["uv", "run", "cxas", "evals", "report", "--help"],
    ),
    ...
    (
        "cxas migrate dfcx --help",
        ["uv", "run", "cxas", "migrate", "dfcx", "--help"],
    ),
]
...
def run_benchmark(iterations: int = 5) -> dict[str, dict[str, float]]:
    ...
    print(
        f"Running CLI performance benchmarks ({iterations} iterations per command)..."
    )
    for label, cmd in COMMANDS_TO_BENCHMARK:
        ...
        if durations:
            ...
            results[label] = stats
            print(
                f" -> mean: {stats['mean_ms']}ms | min: {stats['min_ms']}ms | max: {stats['max_ms']}ms"
            )
    return results
...
def main() -> None:
    ...
    out_file = os.path.join(
        os.path.dirname(__file__), "cli_performance_baselines.json"
    )
    with open(out_file, "w") as f:
        json.dump(results, f, indent=2)
    print(f"\nSaved performance baseline results to {out_file}")
