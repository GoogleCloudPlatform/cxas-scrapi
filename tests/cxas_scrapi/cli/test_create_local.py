...
def test_handle_local_create_agent(
    mock_utils_cls: MagicMock, capsys: Any
) -> None:
    ...
    mock_inst.create_agent.assert_called_once_with(
        display_name="test-item", app_dir="."
    )
    captured = capsys.readouterr()
    assert "Creating local agent template: test-item" in captured.out
    assert (
        "Successfully created local template at: agents/test-item"
        in captured.out
    )


@patch("cxas_scrapi.cli.create_local.CreateUtils", autospec=True)
def test_handle_local_create_tool(
    mock_utils_cls: MagicMock, capsys: Any
) -> None:
    ...
    args = _ns(
        create_local_command="tool", tool_type="http", add_to_agent="agent-a"
    )
    handle_local_create(args)
    ...
    captured = capsys.readouterr()
    assert (
        "Successfully created local template at: tools/test-item"
        in captured.out
    )


@patch("cxas_scrapi.cli.create_local.CreateUtils", autospec=True)
def test_handle_local_create_guardrail(
    mock_utils_cls: MagicMock, capsys: Any
) -> None:
    ...
    captured = capsys.readouterr()
    assert (
        "Successfully created local template at: guardrails/test-item"
        in captured.out
    )


@patch("cxas_scrapi.cli.create_local.CreateUtils", autospec=True)
def test_handle_local_create_exception(
    mock_utils_cls: MagicMock, capsys: Any
) -> None:
