...
def test_combined_evals_report_cmd_success(mocker: Any, capsys: Any, tmp_path: Any) -> None:
    ...
    """
    mock_gen = mocker.patch("cxas_scrapi.utils.reporting.generate_combined_report_from_dir")
...
def test_test_tools_command(mocker: Any, capsys: Any) -> None:
    ...
    mock_tool_evals.run_tool_tests.return_value = pd.DataFrame([{"status": "PASSED"}])
...
def test_test_callbacks_command(mocker: Any, capsys: Any) -> None:
    ...
    Args:
        ...
        capsys: Pytest stdout capture fixture.
    """
    mock_cb_evals_cls = mocker.patch("cxas_scrapi.evals.callback_evals.CallbackEvals")
    mock_cb_evals = mock_cb_evals_cls.return_value
    mock_cb_evals.test_all_callbacks_in_app_dir.return_value = pd.DataFrame([{"status": "PASSED"}])
...
def test_wait_for_evaluation_completion_success(mocker: Any, capsys: Any) -> None:
    ...
    mock_eval_utils.eval_client.get_evaluation_result.return_value = {"result": "ok"}
...
def test_filter_metrics_and_assess(subtests: Any) -> None:
    ...
    with subtests.test("Passed assessment"):
        df_summary = pd.DataFrame([{"evaluation_status": "PASSED", "execution_state": "COMPLETED"}])
        res = evals_cli_module.filter_metrics_and_assess({"summary": df_summary}, filter_auto_metrics=False)
        assert res is True

    with subtests.test("Failed assessment"):
        df_summary = pd.DataFrame([{"evaluation_status": "FAILED", "execution_state": "COMPLETED"}])
        res = evals_cli_module.filter_metrics_and_assess({"summary": df_summary}, filter_auto_metrics=False)
        assert res is False
...
def test_run_eval_prefix_and_tags(mocker: Any, capsys: Any) -> None:
    ...
    mock_utils_cls = mocker.patch("cxas_scrapi.utils.eval_utils.EvalUtils")
    mock_utils = mock_utils_cls.return_value
    mock_utils.evals_to_dataframe.return_value = {"summary": pd.DataFrame([{"evaluation_status": "PASSED", "execution_state": "COMPLETED"}])}
    ...
    mocker.patch("cxas_scrapi.cli.main.wait_for_evaluation_completion", return_value={"summary": pd.DataFrame([{"evaluation_status": "PASSED", "execution_state": "COMPLETED"}])})
...
def test_ci_test_success(mocker: Any, capsys: Any) -> None:
    ...
    mocker.patch("cxas_scrapi.cli.app.app_push", return_value="projects/p/locations/l/apps/ci-app")
    mock_evals_cls = mocker.patch("cxas_scrapi.core.evaluations.Evaluations")
    mock_evals_cls.return_value.get_evaluations_map.return_value = {"goldens": {"g1": "eval_1"}}

    mock_run = mocker.patch("cxas_scrapi.cli.main.subprocess.run")
...
def test_local_test_success(mocker: Any, capsys: Any) -> None:
    ...
    mock_call = mocker.patch("cxas_scrapi.cli.main.subprocess.call", return_value=0)
    args = argparse.Namespace(
        project_id="p",
        location="l",
        app_dir=".",
...
def test_test_single_callback_success(mocker: Any, capsys: Any) -> None:
    ...
    mock_cb_evals_cls = mocker.patch("cxas_scrapi.evals.callback_evals.CallbackEvals")
    mock_cb_evals = mock_cb_evals_cls.return_value
    mock_cb_evals.test_single_callback_for_agent.return_value = pd.DataFrame([{"status": "PASSED"}])
...
def test_export_eval_json_and_output(mocker: Any, capsys: Any, tmp_path: Any) -> None:
    """Verifies export_eval writing JSON to file."""
    from pathlib import Path
    mock_evals_cls = mocker.patch("cxas_scrapi.core.evaluations.Evaluations")
    mock_client = mock_evals_cls.return_value
    mock_client.export_evaluation.side_effect = lambda *a, **kw: (kw.get("output_path") or (a[2] if len(a) > 2 else None)) and Path(kw.get("output_path") or a[2]).write_text("{\"displayName\": \"Eval\"}")
...
def test_wait_for_evaluation_completion_timeout(mocker: Any, capsys: Any) -> None:
    """Verifies wait_for_evaluation_completion raising exit on timeout."""
    mock_eval_utils = MagicMock()
    mock_eval_utils.evals_to_dataframe.return_value = {"summary": pd.DataFrame()}
    with pytest.raises(SystemExit) as exc:
    ...
    assert "Timeout waiting for evaluation to complete." in capsys.readouterr().out
...
def test_evals_error_branches(mocker: Any, capsys: Any) -> None:
    ...
    mock_evals_cls = mocker.patch("cxas_scrapi.core.evaluations.Evaluations")
    mock_evals_cls.return_value.export_evaluation.side_effect = RuntimeError("API down")
    with pytest.raises(SystemExit) as exc:
        evals_cli_module.export_eval(argparse.Namespace(app_name="projects/p/locations/l/apps/a", evaluation_id="id", output=None, format="yaml"))
    assert exc.value.code == 1

    # 2. push_eval no valid goldens
    mocker.patch("cxas_scrapi.utils.eval_utils.EvalUtils.load_golden_evals_from_yaml", return_value=[])
    with pytest.raises(SystemExit) as exc:
        evals_cli_module.push_eval(argparse.Namespace(app_name="projects/p/locations/l/apps/a", file="empty.yaml"))
    assert exc.value.code == 1

    # 3. ci_test app_push failure
    mocker.patch("cxas_scrapi.core.apps.Apps.get_app_by_display_name", return_value=None)
    mocker.patch("cxas_scrapi.cli.app.app_push", return_value=None)
    with pytest.raises(SystemExit) as exc:
        evals_cli_module.ci_test(argparse.Namespace(project_id="p", location="l", display_name="CI", app_dir="."))
    assert exc.value.code == 1

    # 4. local_test docker build failure
    mocker.patch("cxas_scrapi.cli.main.subprocess.call", return_value=1)
    with pytest.raises(SystemExit) as exc:
        evals_cli_module.local_test(argparse.Namespace(project_id="p", location="l", app_dir="."))
    assert exc.value.code == 1


def test_combined_evals_report_cmd_multi_run_and_audio(mocker: Any, capsys: Any, tmp_path: Any) -> None:
...
def test_filter_metrics_and_assess_auto_metrics(capsys: Any) -> None:
    ...
    df_fail = pd.DataFrame([{"record_type": "summary_expectation", "expectation": "exp1", "met_count": 0, "not_met_count": 1}])
    res = evals_cli_module.filter_metrics_and_assess({"expectations": df_fail}, filter_auto_metrics=True)
    assert res is False
    assert "1 custom expectations not met" in capsys.readouterr().out

    # 2. Expectations met
    df_pass = pd.DataFrame([{"record_type": "summary_expectation", "expectation": "exp1", "met_count": 1, "not_met_count": 0}])
    res = evals_cli_module.filter_metrics_and_assess({"expectations": df_pass}, filter_auto_metrics=True)
    assert res is True
...
def test_run_eval_with_wait_and_failures(mocker: Any, capsys: Any) -> None:
    ...
    mock_op.result.return_value = MagicMock()
    mock_client.run_evaluation.return_value = mock_op
    mock_client.list_evaluations.return_value = [MagicMock(name="eval_1", display_name="test_a")]

    mock_utils_cls = mocker.patch("cxas_scrapi.utils.eval_utils.EvalUtils")
    mock_utils = mock_utils_cls.return_value
    mock_utils.evals_to_dataframe.return_value = {"summary": pd.DataFrame([{"evaluation_status": "PASSED"}])}

    df_failures = pd.DataFrame([
        {"display_name": "Eval A", "failure_type": "System Engine Error", "actual": "500 Internal Error"},
        {"display_name": "Eval B", "failure_type": "Mismatch", "expected": "Yes", "actual": "No", "score": 0.2, "turn_index": 1}
    ])
    mocker.patch("cxas_scrapi.cli.main.wait_for_evaluation_completion", return_value={
        "summary": pd.DataFrame([{"evaluation_status": "FAILED", "execution_state": "COMPLETED"}]),
...
def test_tool_and_callback_tests_failures_and_log_files(mocker: Any, capsys: Any, tmp_path: Any) -> None:
    ...
    mock_tool_evals = mock_tool_evals_cls.return_value
    mock_tool_evals.load_tool_test_cases_from_file.return_value = [{"case": 1}]
    mock_tool_evals.run_tool_tests.return_value = pd.DataFrame([{"status": "FAIL", "test_case_id": "tc1", "tool_name": "t1", "error": "err"}])

    args_tool = argparse.Namespace(app_name="projects/p/locations/l/apps/a", test_file="tools.json", debug=True)
    with pytest.raises(SystemExit) as exc:
    ...
    mock_cb_evals_cls = mocker.patch("cxas_scrapi.evals.callback_evals.CallbackEvals")
    mock_cb_evals = mock_cb_evals_cls.return_value
    mock_cb_evals.test_all_callbacks_in_app_dir.side_effect = lambda *a, **kw: (Path(kw["log_file"]).write_text("cb log") if kw.get("log_file") else None) or pd.DataFrame([{"status": "FAILED"}])

    log_file = tmp_path / "cb.log"
    args_cb = argparse.Namespace(
        app_dir="/tmp/app", agent_name="agent_a", callback_type="before_model", callback_name="cb1", log_file=str(log_file), pytest_args=["-v"]
    )
    with pytest.raises(SystemExit) as exc:
    ...
    mock_cb_evals.test_single_callback_for_agent.side_effect = lambda *a, **kw: (Path(kw["log_file"]).write_text("single log") if kw.get("log_file") else None) or pd.DataFrame([{"status": "FAILED"}])
    log_single = tmp_path / "single.log"
    args_single = argparse.Namespace(
        app_name="a", agent_name="ag", callback_type="before_model", test_file_path="cb.py", log_file=str(log_single), pytest_args=None
    )
    with pytest.raises(SystemExit) as exc:
...
def test_ci_test_and_local_test_full_paths_and_env(mocker: Any, capsys: Any, tmp_path: Any) -> None:
    ...
    mocker.patch("cxas_scrapi.cli.app.app_push", return_value="projects/p/locations/l/apps/ci-app")
    mock_run = mocker.patch("cxas_scrapi.cli.main.subprocess.run", return_value=MagicMock(returncode=0))

    app_dir = tmp_path / "app"
    (app_dir / "tests").mkdir(parents=True, exist_ok=True)
    (app_dir / "tests" / "tool_tests.yaml").write_text("tests: []")

    args_ci = argparse.Namespace(project_id="p", location="l", app_dir=str(app_dir), display_name="CI", env_file=None)
    evals_cli_module.ci_test(args_ci)
    assert mock_run.call_count >= 1

def test_wait_for_evaluation_completion_full_polling_loop(mocker: Any, capsys: Any) -> None:
    """Test wait_for_evaluation_completion handling empty summary initially, then error states."""
    mock_eval_utils = MagicMock()
    df_empty = pd.DataFrame()
    df_err = pd.DataFrame([{"eval_result_id": "run_x", "execution_state": "ERROR"}])
    mock_eval_utils.evals_to_dataframe.side_effect = [{"summary": df_empty}, {"summary": df_err}]
    mock_eval_utils.eval_client.get_evaluation_result.return_value = {"result": "error"}
...
def test_ci_test_with_running_tools_and_evals(mocker: Any, capsys: Any, tmp_path: Any) -> None:
    ...
    mock_evals_cls = mocker.patch("cxas_scrapi.core.evaluations.Evaluations")
    mock_evals_cls.return_value.get_evaluations_map.return_value = {"goldens": {"g1": "eval_1"}, "scenarios": {"s1": "sim_1"}}

    app_dir = tmp_path / "app"
    (app_dir / "tests").mkdir(parents=True, exist_ok=True)
    (app_dir / "tests" / "tool_tests.yaml").write_text("tests: []")

    args = argparse.Namespace(project_id="p", location="l", app_dir=str(app_dir), display_name="CI", env_file="env.json")
    evals_cli_module.ci_test(args)
    assert mock_run.call_count >= 3  # 1 for test-tools, 2 for run (golden + scenario)


def test_local_test_full_paths_and_env(mocker: Any, capsys: Any, tmp_path: Any) -> None:
    """Test local_test when env_file and oauth_token are present."""
    app_dir = tmp_path / "app"
    app_dir.mkdir(parents=True, exist_ok=True)
    mocker.patch("cxas_scrapi.cli.main.subprocess.call", return_value=0)
    mocker.patch.dict("os.environ", {"CXAS_OAUTH_TOKEN": "secret_token"})
    args_local = argparse.Namespace(project_id="p", location="l", app_dir=str(app_dir), env_file="env.json")
    with pytest.raises(SystemExit) as exc:
