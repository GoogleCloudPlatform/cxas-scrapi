...
@patch("cxas_scrapi.core.common.Common._get_app_name", return_value="projects/p/locations/l/apps/a")
@patch("cxas_scrapi.core.tools.Tools")
def test_tools_delete_display_name_and_errors(mock_tools_cls, mock_get_app, capsys):
    mock_tools = mock_tools_cls.return_value
    t1 = FakeProto(name="projects/p/locations/l/apps/a/tools/t1", display_name="my_tool")
    mock_tools.list_tools.return_value = [t1]
    mock_tools.get_tools_map.return_value = {"my_tool": "projects/p/locations/l/apps/a/tools/t1"}

    # 1. Delete by display name
    resources_cli.tools_delete(_ns(tools_command="delete", name="my_tool"))
    mock_tools.delete_tool.assert_called_once_with("projects/p/locations/l/apps/a/tools/t1")

    # 2. Delete exception
    mock_tools.delete_tool.side_effect = RuntimeError("Tool locked")
    with pytest.raises(SystemExit) as exc:
        resources_cli.tools_delete(_ns(tools_command="delete", name="projects/p/locations/l/apps/a/tools/t1"))
...
def test_callbacks_list_and_delete_branches(mock_cb_cls, mock_agents_cls, mock_get_app, capsys):
    ...
    ag_mock = FakeProto(name="projects/p/locations/l/apps/a/agents/ag1", display_name="ag1")
    mock_agents.get_agent.return_value = ag_mock
    mock_agents.get_agents_map.return_value = {"ag1": "projects/p/locations/l/apps/a/agents/ag1"}
    mock_agents.list_agents.return_value = [ag_mock]

    cb_item = FakeProto(uri="https://cb.com", display_name="my_cb", python_code="def cb(): pass\n", description="Test cb")
    mock_cb.list_callbacks.return_value = {"before_model": [cb_item]}

    # 1. list with agent and cb_type filter
    resources_cli.callbacks_list(_ns(callbacks_command="list", agent_name="projects/p/locations/l/apps/a/agents/ag1", callback_type="before_model"))
    assert "Test cb" in capsys.readouterr().out

    # 2. delete by index
    resources_cli.callbacks_delete(_ns(callbacks_command="delete", agent_name="ag1", callback_type="before_model", index=0))
...
def test_resources_fallback_and_error_branches(mock_vars_cls: MagicMock, mock_cb_cls: MagicMock, mock_agents_cls: MagicMock, mock_tools_cls: MagicMock, mock_get_app: MagicMock, capsys: Any) -> None:
    """Test trailing ID lookup fallbacks and exception handling across resources_cli."""
    mock_tools = mock_tools_cls.return_value
    t1 = FakeProto(name="projects/p/locations/l/apps/a/tools/t1", display_name="t1")
    mock_tools.get_tools_map.return_value = {}
    mock_tools.list_tools.return_value = [t1]
    resources_cli.tools_delete(_ns(tools_command="delete", name="t1"))
    mock_tools.delete_tool.assert_called_with("projects/p/locations/l/apps/a/tools/t1")

    mock_agents = mock_agents_cls.return_value
    ag1 = FakeProto(name="projects/p/locations/l/apps/a/agents/ag1", display_name="ag1")
    mock_agents.get_agents_map.return_value = {}
    mock_agents.list_agents.return_value = [ag1]
    mock_cb = mock_cb_cls.return_value
    mock_cb.list_callbacks.return_value = {}
    resources_cli.callbacks_list(_ns(callbacks_command="list", agent_name="ag1"))
...
def test_tools_error_and_not_found(mock_tools_cls, mock_get_app, capsys):
    ...
    mock_tools.get_tools_map.return_value = {}
    with pytest.raises(SystemExit) as exc:
        resources_cli.tools_delete(_ns(tools_command="delete", name="non_existent"))
    assert exc.value.code == 1
...
@patch("cxas_scrapi.core.common.Common._get_app_name", return_value="projects/p/locations/l/apps/a")
@patch("cxas_scrapi.core.variables.Variables")
def test_variables_list_and_delete_branches(mock_vars_cls, mock_get_app, capsys):
    """Test variables listing and deletion success and error branches."""
    mock_vars = mock_vars_cls.return_value
    mock_vars.variable_to_dict.side_effect = lambda v: {"name": v.name, "displayName": v.display_name, "description": "ID", "type": "STRING"}
    v1 = FakeProto(name="projects/p/locations/l/apps/a/variables/v1", display_name="user_id", description="ID", type="STRING", schema=FakeProto(type_=1))
    ...
    resources_cli.variables_delete(_ns(variables_command="delete", name="user_id"))
...
@mock.patch("cxas_scrapi.core.variables.Variables")
def test_variables_delete_success(mock_vars_cls: MagicMock, capsys: Any) -> None:
