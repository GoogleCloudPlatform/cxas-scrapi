...
from cxas_scrapi.cli.versions_cli import app_versions_compare, app_versions_list
...
def test_app_versions_list_success(
    mock_resolve: MagicMock, mock_versions_cls: MagicMock
) -> None:
...
@patch("cxas_scrapi.cli.versions_cli.Versions")
@patch("cxas_scrapi.cli.versions_cli._resolve_app_args")
def test_app_versions_list_empty(
    mock_resolve: MagicMock, mock_versions_cls: MagicMock
) -> None:
...
@patch("cxas_scrapi.cli.versions_cli.Versions")
@patch("cxas_scrapi.cli.versions_cli._resolve_app_args")
def test_app_versions_list_exception(
    mock_resolve: MagicMock,
    mock_versions_cls: MagicMock,
...
@patch("cxas_scrapi.cli.versions_cli.Versions")
@patch("cxas_scrapi.cli.versions_cli._resolve_app_args")
def test_app_versions_compare_summary_and_html(
    mock_resolve: MagicMock, mock_versions_cls: MagicMock, mock_html: MagicMock
) -> None:
    ...
    tool1 = FakeProto(
        name="tool_a", display_name="Tool A", description="Find orders"
    )
...
@patch("cxas_scrapi.cli.versions_cli.Versions")
@patch("cxas_scrapi.cli.versions_cli._resolve_app_args")
def test_app_versions_compare_verbose_and_markdown(
    mock_resolve: MagicMock,
    mock_versions_cls: MagicMock,
    ...
    agent1 = FakeProto(
        name="agent_a", display_name="Agent A", instruction="Hello", tools=[]
    )
    agent2 = FakeProto(
        name="agent_b", display_name="Agent B", instruction="Hello B", tools=[]
    )
...
@patch("cxas_scrapi.cli.versions_cli.Versions")
@patch("cxas_scrapi.cli.versions_cli._resolve_app_args")
def test_app_versions_compare_html_report_execution(
    mock_resolve: MagicMock, mock_versions_cls: MagicMock, tmp_path: Any
) -> None:
    ...
    agent2 = FakeProto(
        name="agent_a",
        display_name="Agent A",
        instruction="Be more helpful",
        tools=["tools/tool_b"],
    )
    agent3 = FakeProto(
        name="agent_c",
        display_name="Agent C",
        instruction="New agent",
        tools=[],
    )

    tool1 = FakeProto(
        name="tool_a", display_name="Tool A", description="Old tool"
    )
    tool2 = FakeProto(
        name="tool_a", display_name="Tool A", description="Modified tool"
    )
    tool3 = FakeProto(
        name="tool_b", display_name="Tool B", description="Added tool"
    )
    ...
    ts2 = FakeProto(
        name="ts_a", display_name="TS A", tools=["tool_a", "tool_b"]
    )
    ...
    assert (
        "Version Comparison Report" in content
        or "Version Diff" in content
...
@patch("cxas_scrapi.cli.versions_cli.Versions")
@patch("cxas_scrapi.cli.versions_cli._resolve_app_args")
def test_app_versions_compare_full_drift_summary(
    mock_resolve: MagicMock, mock_versions_cls: MagicMock, mock_html: MagicMock
) -> None:
    ...
    tool_add = FakeProto(
        name="t2", display_name="Tool 2", description="Added tool"
    )
...
@patch("cxas_scrapi.cli.versions_cli.Versions")
@patch("cxas_scrapi.cli.versions_cli._resolve_app_args")
def test_app_versions_compare_exception(
    mock_resolve: MagicMock,
    mock_versions_cls: MagicMock,
    mock_console: MagicMock,
) -> None:
