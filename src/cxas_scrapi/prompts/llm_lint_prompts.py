...
2. INSTRUCTION STYLE & COHESION:
   ...
   - Variable Mutation & Updates: Check whether instructions instruct or attempt to have the agent update, set, assign, store, or modify variable values (e.g., "update variable {user_name} to 'John'", "set {is_verified} to true", "store the account number in {account_num}"). Flag any instructions attempting to update variable values in natural language, because the agent itself cannot update the value of a variable (see https://docs.cloud.google.com/customer-engagement-ai/conversational-agents/ps/variable#updating_variable_values). Instruct that state modifications should happen in Python tools or callbacks. Ensure you do not falsely trigger on instructions that merely read or reference existing variable values (e.g., "greet the user by {user_name}", "if {is_verified} is true").
...
- INSTRUCTION STYLE & COHESION: Detailed review of length, task decomposition, completeness, ambiguity, contradictions/alignment, quotes vs. back-ticks, variable/entity formatting, and variable mutation/updates, pointing out specific instructions and explaining how to correct them. Provide a concrete rewrite suggestion for the problematic sections using proper nested numbering.
