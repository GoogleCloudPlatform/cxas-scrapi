...
def workspace_set(args: Any) -> None:
...
def workspace_show(args: Any) -> None:
...
def workspace_create(args: Any) -> None:
...
def workspace_unset(args: Any) -> None:
...
def workspace_group() -> None:
    """Manage GECX workspaces, active project pointers, and profiles."""
    pass


@workspace_group.command(name="show")
@click.pass_context
def workspace_show_cmd(ctx: click.Context, **kwargs: Any) -> None:
    """Show resolved workspace path, config file, active profile, and settings."""
    workspace_show(ctx)


@workspace_group.command(name="set")
@click.argument("pos_target_dir", required=False)
@click.option("--target-dir", help="Target directory path.")
@click.option("--profile", help="Active configuration profile under [profiles].")
@click.option("--gcp-project-id", help="Target GCP Project ID.")
@click.option("--deployed-app-id", help="Target deployed app ID.")
@click.option("--location", help="GCP location string.")
@click.option("--app-dir", help="App directory path.")
@click.option("--evals-dir", help="Evaluations directory path.")
@click.option("--output-dir", help="Output directory path.")
@click.option("--model", help="Gemini model name.")
@click.option("--modality", help="Default session modality.")
@click.pass_context
def workspace_set_cmd(ctx: click.Context, pos_target_dir: str | None = None, **kwargs: Any) -> None:
    """Set active project pointer or update profile options."""
    if pos_target_dir and not kwargs.get("target_dir"):
        kwargs["target_dir"] = pos_target_dir
    cfg = to_dataclass(WorkspaceSetConfig, ctx, **kwargs)
    workspace_set(cfg)
...
@click.option("--target-dir", help="Target directory path.")
@click.pass_context
def workspace_create_cmd(ctx: click.Context, pos_target_dir: str | None = None, **kwargs: Any) -> None:
    """Create a default gecx-config.toml in the target directory."""
    class _Args:
        pass
    a = _Args()
    a.target_dir = kwargs.get("target_dir") or pos_target_dir or "."
    workspace_create(a)


@workspace_group.command(name="unset")
@click.pass_context
def workspace_unset_cmd(ctx: click.Context, **kwargs: Any) -> None:
    """Unset active project pointer file."""
    workspace_unset(None)
