...
def _to_kebab_case_keys(d: dict[str, Any]) -> dict[str, Any]:
    """Recursively convert all dictionary keys from snake_case to kebab-case."""
    res = {}
    for k, v in d.items():
        new_k = k.replace("_", "-")
        if isinstance(v, dict):
...
def _migrate_pointer(dir_path: pathlib.Path) -> None:
    ...
    # 1. If legacy file exists, move its content to active-project in TOML
    if legacy_file.is_file():
        try:
            ...
            legacy_file.unlink()
            warnings.warn(
                "Legacy pointer file '.active-project' is deprecated. "
                "Seamlessly migrated to '.scrapi/active-project'. "
                "Please commit the new '.scrapi/' folder.",
    ...
    if active_project_file.is_file():
        try:
            ...
            if is_json:
                ...
                warnings.warn(
                    "Pointer file '.scrapi/active-project' was in JSON format, "
                    "which is deprecated. Seamlessly migrated to TOML format.",
                    category=UserWarning,
                    stacklevel=2,
                )
        except Exception as e:
    ...
    if json_file.is_file() and not toml_file.exists():
        try:
            ...
            warnings.warn(
                ...
                " Seamlessly migrated to 'gecx-config.toml'. "
                "Please commit the new TOML file.",
                category=UserWarning,
                stacklevel=2,
            )
        except Exception as e:
            logger.warning(
                "Failed to silently migrate gecx-config.json to "
                "gecx-config.toml: %s",
                e,
            )
...
def _parse_pointer_as_toml(content: str) -> str | None:
    """Attempts to parse pointer file content as TOML.

    Returns base-dir if found.
    """
    try:
        data = toml.loads(content)
        if isinstance(data, dict) and "base-dir" in data:
...
def _parse_pointer_as_json(content: str) -> str | None:
    """Attempts to parse pointer file content as JSON.
...
def find_workspace_root() -> str | None:
    """Find the workspace root by crawling up from CWD.

    Crawls upward until a workspace marker directory is found.
    """
...
def resolve_project_dir() -> str:
    ...
    raise ValueError(
        ...
        "Please set one using 'cxas workspace set [path-to-project-dir]',\n"
        "run this command from within a GECX project directory containing "
        "gecx-config.json, or initialize a new project using 'cxas init'."
    )
...
def get_workspace_path(*parts) -> str:
    ...
    Raises:
        SystemExit: If called when no SCRAPI workspace pointer can be
...
def project_path(*parts) -> str:
    ...
    A utility function to construct absolute file paths anchored to the
    currently active GECX project directory (as resolved by
    `resolve_project_dir()`).
    ...
    Returns:
        str: The absolute path constructed by joining the active project
            directory and the provided parts.
    """
...
def _resolve_profile_overrides(
    ...
    """Traverse and resolve overrides for a dot-separated profile path."""
...
def load_workspace_config() -> dict[str, Any]:
    ...
    if config_file_toml.exists():
        ...
        raise ValueError(
            f"Neither gecx-config.toml nor gecx-config.json found in "
            f"{project_dir}. Run 'cxas init' to create a default config."
        )

    # Extract baseline config from 'default' section, fallback to raw config
    ...
    if not project or not app_id:
        raise ValueError(
            f"{config_file.name} missing 'gcp_project_id' or "
            "'deployed_app_id'. Update it with 'cxas workspace set'."
        )
...
def get_output_dir() -> str:
    ...
    This function determines where evaluation reports and generated
    artifacts should be saved. It reads the `output_dir` property from the
    active project's configuration (defaulting to '.scrapi-out'). If the
    configured path is absolute, it is returned directly; otherwise, it is
    resolved relative to the active project directory.
...
def create_default_config(target_dir: str) -> None:
    """Create a default gecx-config.toml file in the target directory.
...
def update_workspace_config(updates: dict[str, Any]) -> tuple[bool, str]:
    """Update the active project's config file with the provided updates.
    ...
    # Check if config has default section, target modifications there
    ...
    if updated:
        # Always write back to gecx-config.toml as TOML with kebab-case
    ...
    def wrapper(*args, **kwargs):
        if not find_workspace_root():
            raise ValueError(
                ...
                "Profiles can only be managed within an active workspace."
            )
