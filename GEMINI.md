...
3. **Authorized Specialized Sub-Agents:** You are explicitly authorized and encouraged to use specialized sub-agents provided by the `cx-agent-studio` skill (such as `lint-fixer`, `eval-writer`, `coverage-analyst`, `tdd-writer`, and `triage-failure`) for their exact designated purposes to save main-thread context and execute repetitive tasks efficiently.
