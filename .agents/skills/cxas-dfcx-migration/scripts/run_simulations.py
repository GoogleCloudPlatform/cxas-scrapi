...
def main():
    ...
    if not yaml_files:
        console.print(f"[yellow]No YAML files matching '{args.filter}' in {args.sim_dir}[/]")
        sys.exit(0)

    console.print(f"Found {len(yaml_files)} simulation file(s): {', '.join(yaml_files)}")
    ...
    console.print(f"\n[bold]Running {len(all_tests)} simulations against {args.app_name}[/]\n")
    ...
    table.add_row("Pass rate", f"{passed / total * 100:.1f}%" if total else "N/A")
    console.print(table)
