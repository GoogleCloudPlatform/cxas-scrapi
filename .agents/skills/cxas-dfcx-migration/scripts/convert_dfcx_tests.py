...
from cxas_scrapi.evals.turn_evals import TurnTestCase
...
def parse_args():
    ...
    source.add_argument(
        "--ir-bundle",
        help="Path to IR bundle JSON (contains source test cases and agent mapping)",
    )
    source.add_argument(
        "--source-dir",
    ...
    parser.add_argument(
        "--output-dir",
        default=None,
        help="Output directory for YAML files (default: <target>_evals/simulations)",
    )
    ...
    for f in sorted(os.listdir(source_dir)):
        if f.endswith(".json"):
            with open(os.path.join(source_dir, f)) as fh:
                test_cases.append(json.load(fh))
...
def main():
    ...
    table.add_row("CONTAINS assertions", str(report["contains_assertions"]))
    table.add_row("AGENT_TRANSFER assertions", str(report["agent_transfer_assertions"]))
