...
2.  **Initialize Checklist**: Call the checklist tool to initialize a new
    checklist. `bash python3 scripts/manage_checklist.py init --title "My Task
    Title"`
3.  **Add Items**: Iterate through the listed items and add each one to the
    ...
    items. `bash python3 scripts/manage_checklist.py add --item "item_name"
    --output_check_path "path/to/output"`
...
6.  **Mark Done**: When an item is processed and output produced, call the tool
    to mark it as done. `bash python3 scripts/manage_checklist.py done --item
    "item_name"`
...
3.  Call the `done` command on the primary checklist only after

the subagent reports success and you verify their output.
