...
"""Interactive configuration wizard for gecx-config.toml.
...
def _resolve_config_dir():
    """Return the directory where gecx-config.toml and cxas_app/ should live."""
...
def _config_file():
    return os.path.join(_resolve_config_dir(), "gecx-config.toml")
...
def banner():
    ...
    console.print(
        Panel(
            ...
            " [cyan]gecx-config.toml[/cyan] with your project settings.",
            box=box.DOUBLE,
            style="blue",
        )
    )
...
def review_and_confirm(config):
    ...
    confirmed = inquirer.confirm(
        message="Write this configuration to gecx-config.toml?",
        default=True,
        style=PROMPT_STYLE,
    ).execute()
...
def load_existing_config():
    ...
    if os.path.exists(CONFIG_FILE):
        try:
            ...
            pass
    json_file = os.path.join(_resolve_config_dir(), "gecx-config.json")
...
def save_config(config):
    """Save configuration dictionary to gecx-config.toml."""
