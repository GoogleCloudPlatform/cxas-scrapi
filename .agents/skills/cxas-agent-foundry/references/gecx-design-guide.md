...
Write the same style prompt into each locale, changing only the accent line. The pair below is a real shipped configuration copied out of a live app rather than an illustration, and a line diff of the two strings returns exactly one hunk, the accent line:
