...
| **T014** (deprecated standalone `OpenApiTool` in `tools/`) | Standalone `OpenApiTool`s (`open_api_tool/` or `"openApiTool"`) are deprecated in CES runtime and fail during conversational turns. Move `tools/<name>/open_api_tool/open_api_schema.yaml` to `toolsets/<ToolsetDisplayName>/open_api_toolset/open_api_schema.yaml`, create `<ToolsetDisplayName>.json` with `"openApiToolset"`, and update `agent.json` to declare `"toolsets": [{"toolset": "<ToolsetDisplayName>", "toolIds": [...]}]`. |
