...
def parse_dfcx_zip(zip_path: str) -> List[Dict[str, Any]]:
    ...
    with zipfile.ZipFile(zip_path, "r") as z:
        ...
        for name in z.namelist():
            ...
            if len(parts) == 3 and parts[0] == "intents" and parts[2].endswith(".json") and parts[2].replace(".json", "") == parts[1]:
                intent_definition_files.append(name)
            # Look for intents/<intent_dir>/trainingPhrases/<lang>.json
            elif len(parts) == 4 and parts[0] == "intents" and parts[2] == "trainingPhrases" and parts[3].endswith(".json"):
                intent_dir = parts[1]
        ...
        for def_file in intent_definition_files:
            ...
            with z.open(def_file) as f:
                try:
                    ...
                    print(f"Warning: Failed to parse {def_file}: {e}", file=sys.stderr)
                    continue
                ...
                # Collect training phrases (usually just English 'en' or first available)
                phrases = []
                ...
                if tp_files:
                    ...
                    with z.open(tp_files[0]) as tpf:
                        try:
                            ...
                            print(f"Warning: Failed to parse training phrases in {tp_files[0]}: {e}", file=sys.stderr)

                # Keep a maximum of 5 representative training phrases to keep prompt context clean
...
def main():
    parser = argparse.ArgumentParser(description="Parse DFCX agent zip and print intent list for LLM context.")
    parser.add_argument("--zip-path", required=True, help="Path to the DFCX export .zip file.")
